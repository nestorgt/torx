...
function menuListPendingPayouts() {
  /*
   * Menu handler to display pending payouts list
   */
  try {
    var ui = SpreadsheetApp.getUi();
    
    // Get pending payouts data
    var payoutsData = listPendingPayouts();
    
    if (!payoutsData.success) {
      ui.alert('Error', 'Failed to get pending payouts: ' + payoutsData.error, ui.ButtonSet.OK);
      return;
    }
    
    // Format and display the list
    var message = formatPendingPayoutsList(payoutsData);
    ui.alert('Pending Payouts', message, ui.ButtonSet.OK);
    
  } catch (e) {
    Logger.log('[ERROR] Menu list pending payouts failed: %s', e.message);
    SpreadsheetApp.getUi().alert('Error', 'Failed to list pending payouts: ' + e.message, SpreadsheetApp.getUi().ButtonSet.OK);
  }
}
